...
trait EmbeddedMongo {
  ...
  def withRunningEmbeddedMongo[F[_]: Async, A](
    ...
    EmbeddedMongo
      .start[F](mongoPort, Some(mongoUsername), Some(mongoPassword), mongoVersion)
      .use(_ => test)
...
object EmbeddedMongo {
  ...
  private def startMongod(
    ...
    Mongod
      ...
      .mongodArguments(
        Start
          .to(classOf[MongodArguments])
          .initializedWith(MongodArguments.defaults().withAuth(withAuth))
      )
      .build()
        ...
        { runningProcess =>
          val createUser = new Document("createUser", username)
            ...
            .append(
              "roles",
              java.util.Arrays
                .asList("userAdminAnyDatabase", "dbAdminAnyDatabase", "readWriteAnyDatabase")
            )
